...
class ResNetInput_Cifar10(object):
    def __init__(self, batch_size, data_dir=r'D:\ml\datasets\cifar-10-batches-bin'):
        self.data_dir = data_dir
        self.batch_size = batch_size
    ...
    def destroy_image(self, image, training=False):
        """
        destroy images
        """
        if training:
    ...
    def distorted_inputs(self):
        ...
        read_input.label = tf.cast(read_input.label, tf.int64)

        destroyed_image = self.destroy_image(reshaped_image, True)
    ...
    def inputs(self, eval_data):
        ...
        destroyed_image = self.destroy_image(reshaped_image, False)
...
if __name__ == '__main__':
    batch_size = 10
    resnet_input = ResNetInput_Cifar10(batch_size)
    images, labels = resnet_input.inputs(True)
    ...
    with tf.Session() as sess:
        ...
        try:
            ...
            while True:
                ...
                print(_images.shape, _labels.shape)
                for i in range(batch_size):
                    plt.imshow(_images[i]), plt.title('{0}'.format(_labels[i]))
                    plt.show()
        except:
            coord.request_stop()
        coord.request_stop()
